...
export default function CreateLimitOrder({ abi, handleTransactionSuccess }) {
  ...
  const encrypt = async (e, usdcAmount, targetPrice) => {
    ...
    const payloadHash = keccak256(
      concat([text_to_bytes('\x19Ethereum Signed Message:\n32'), arrayify(ciphertextHash)])
    );
    ...
    try {
      ...
      handleTransactionSuccess(txHash); // Call the success handler with the transaction hash
    } catch (error) {
